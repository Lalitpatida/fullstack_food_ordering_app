...
{cart?.length >0 && (
    ...
    <p className=" text-cyan-50 text-base font-semibold">{cart?.length}</p>
</div>
)}
</motion.div>
...
<><div className=' relative cursor-pointer' onMouseEnter={()=> setIsMenu(true)}>
    ...
    <div className=' w-12 h-12 rounded-full shadow-md cursor-pointer overflow-hidden flex items-center justify-center'>
        ...
        src= {user && user.picture ? user.picture : Avatar} whileHover={{scale: 1.15}}
    ...
    onMouseLeave={()=>setIsMenu(false)} className='px-6 py-4 w-48 backdrop-blur-md rounded-md shadow-md absolute bg-lightOverlay top-12 right-0 flex flex-col gap-4'>
   ...
   <Link className=' hover:text-red-500 text-xl text-textColor'to={"/dashboard/home"}>Dashboard
        ...
        {...buttonClick} 
        className="group flex items-center justify-center px-3 py-2 rounded-md shadow-md bg-gray-100 hover:bg-green-200 gap-3">
